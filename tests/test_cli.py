...
class TestBuildParser:
    ...
    def test_diff_session_custom(self):
        parser = build_parser()
        args = parser.parse_args(["diff", "--session", "7"])
    ...
    def test_all_subcommands_registered(self):
        parser = build_parser()
        commands = [
            "health", "stats", "diff", "changelog", "coverage",
            "score", "arch", "refactor", "replay", "plan", "triage",
            "depgraph", "todos", "doctor", "run",
            "timeline", "coupling", "complexity",
        ]
        for cmd in commands:
    ...
    def test_todos_session_flag(self):
        parser = build_parser()
        args = parser.parse_args(["todos", "--session", "5"])
        assert args.session == 5

    def test_todos_threshold_flag(self):
        parser = build_parser()
        args = parser.parse_args(["todos", "--threshold", "3"])
...
class TestMain:
    ...
    def test_main_coverage_missing_file(self, tmp_path, capsys):
        """Should return exit 1 if coverage history file doesn't exist."""
    ...
    def test_main_diff(self, tmp_path):
        mock_diff = MagicMock()
        mock_diff.to_markdown = lambda: "# diff\n"
...
class TestHelpers:
    """Tests for the print helper functions."""

    def test_print_header_outputs(self, capsys):
        _print_header("Test Title")
        captured = capsys.readouterr()
        assert "Test Title" in captured.out
        assert "🌙" in captured.out or "Nightshift" in captured.out
...
class TestReplaySubcommand:
    """Tests for nightshift replay."""

    def test_replay_parses_session(self):
        parser = build_parser()
        args = parser.parse_args(["replay", "--session", "5"])
        assert args.session == 5
    ...
    def test_replay_specific_session_not_found(self, tmp_path):
        """Should return 1 when session not found."""
        log = tmp_path / "NIGHTSHIFT_LOG.md"
...
class TestPlanSubcommand:
    """Tests for nightshift plan."""

    def test_plan_parses_session(self):
        parser = build_parser()
        args = parser.parse_args(["plan", "--session", "3"])
...
class TestTriageSubcommand:
    ...
    def test_triage_with_issues_file(self, tmp_path):
        ...
        with patch("src.issue_triage.load_issues", return_value=[]), \
             patch("src.issue_triage.triage_issues", return_value=mock_triaged), \
...
class TestDepgraphSubcommand:
    """Tests for nightshift depgraph."""

    def test_depgraph_parses(self):
        parser = build_parser()
        args = parser.parse_args(["depgraph"])
...
class TestTodosSubcommand:
    """Tests for nightshift todos."""

    def test_todos_parses(self):
        parser = build_parser()
        args = parser.parse_args(["todos"])
...
class TestTimelineSubcommand:
    ...
    def test_timeline_renders_output(self, tmp_path, capsys):
        """Should print timeline without error."""
