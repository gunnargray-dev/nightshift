"""Tests for the Nightshift changelog generator (src/changelog.py)."""
...
class TestCommitRecord:
    def test_defaults(self):
        cr = CommitRecord(
            sha="abc1234",
            subject="[nightshift] feat: add stats",
            commit_type="feat",
    ...
    def test_to_dict(self):
        cr = CommitRecord(
            sha="abc1234",
            subject="[nightshift] fix: fix bug",
            commit_type="fix",
            description="fix bug",
            session=2,
            body="Session: 2",
...
class TestChangelogSection:
    def _make_commit(self, ctype: str = "feat", desc: str = "something") -> CommitRecord:
        return CommitRecord(
            sha="abc1234",
            subject=f"[nightshift] {ctype}: {desc}",
            commit_type=ctype,
            description=desc,
            session=1,
        )
...
class TestChangelog:
    def _make_section(self, session: int, types: dict[str, int]) -> ChangelogSection:
        ...
        for ctype, count in types.items():
            commits_by_type[ctype] = [
                CommitRecord(
                    sha=f"abc{i:04d}",
                    subject=f"[nightshift] {ctype}: item {i}",
                    commit_type=ctype,
    ...
    def test_empty_changelog(self):
        ...
        assert "No Nightshift commits found" in md

    def test_to_markdown_has_header(self):
        cl = Changelog(repo_name="nightshift", generated_at="2026-02-27")
        md = cl.to_markdown()
        assert "# Changelog" in md
        assert "nightshift" in md
...
class TestPatterns:
    def test_subject_pattern_matches(self):
        m = SUBJECT_PATTERN.match("[nightshift] feat: add stats engine")
        assert m is not None
        assert m.group(1) == "feat"
        assert m.group(2) == "add stats engine"

    def test_subject_pattern_case_insensitive(self):
        m = SUBJECT_PATTERN.match("[Nightshift] FEAT: Add Thing")
        assert m is not None

    def test_subject_pattern_no_match_non_nightshift(self):
...
class TestParseCommitLog:
    def _make_raw_entry(
        ...
        subject: str = "[nightshift] feat: add thing",
        body: str = "Session: 1",
    ) -> str:
    ...
    def test_ignores_non_nightshift_commits(self):
        raw = (
            self._make_raw_entry(subject="chore: cleanup") +
            self._make_raw_entry(subject="[nightshift] fix: bug fix")
        )
        records = parse_commit_log(raw)
        assert len(records) == 1
    ...
    def test_parses_multiple_commits(self):
        raw = (
            self._make_raw_entry(sha="aaa", subject="[nightshift] feat: feature A", body="Session: 1") +
            self._make_raw_entry(sha="bbb", subject="[nightshift] fix: fix B", body="Session: 2") +
            self._make_raw_entry(sha="ccc", subject="[nightshift] ci: add CI", body="Session: 1")
        )
        records = parse_commit_log(raw)
        assert len(records) == 3

    def test_strips_sha_and_subject(self):
        raw = self._make_raw_entry(sha="abc123full", subject="[nightshift] docs: update readme")
        records = parse_commit_log(raw)
    ...
    def test_handles_entry_with_only_two_parts(self):
        raw = "abc1234\x00[nightshift] feat: thing\x1e"
        records = parse_commit_log(raw)
        assert len(records) == 1
...
class TestGroupBySession:
    def _make_commit(self, ctype: str, session: int) -> CommitRecord:
        return CommitRecord(
            sha="abc1234",
            subject=f"[nightshift] {ctype}: thing",
            commit_type=ctype,
            description="thing",
            session=session,
        )
...
class TestGenerateChangelog:
    ...
    def test_parses_real_log(self, tmp_path):
        raw = (
            "abc1234\x00[nightshift] feat: add stats engine\x00Session: 1\x1e"
            "def5678\x00[nightshift] ci: add GitHub Actions\x00Session: 1\x1e"
        )
    ...
    def test_integration_with_real_git(self, tmp_path):
        """Integration: creates a real git repo with Nightshift commits."""
        ...
        subprocess.run(
            ["git", "commit", "-m", "[nightshift] feat: add thing\n\nSession: 1"],
            cwd=tmp_path,
            capture_output=True,
        )
        cl = generate_changelog(repo_path=tmp_path)
                ...
                ChangelogSection(
                    ...
                    commits_by_type={
                        "feat": [
                            CommitRecord(
                                sha="abc1234",
                                subject="[nightshift] feat: add thing",
                                commit_type="feat",
