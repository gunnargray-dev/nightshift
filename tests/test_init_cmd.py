...
class TestBootstrap:
    def test_creates_nightshift_toml(self, tmp_path):
        ic = _import()
        ic.bootstrap(tmp_path)
        assert (tmp_path / "nightshift.toml").exists()

    def test_creates_nightshift_log(self, tmp_path):
        ic = _import()
        ic.bootstrap(tmp_path)
        assert (tmp_path / "NIGHTSHIFT_LOG.md").exists()
    ...
    def test_creates_github_workflow(self, tmp_path):
        ic = _import()
        ic.bootstrap(tmp_path)
        assert (tmp_path / ".github" / "workflows" / "nightshift.yml").exists()

    def test_workflow_content_is_valid_yaml_shape(self, tmp_path):
        ic = _import()
        ic.bootstrap(tmp_path)
        content = (tmp_path / ".github" / "workflows" / "nightshift.yml").read_text()
        assert "nightshift health" in content
        assert "python-version" in content

    def test_nightshift_toml_has_thresholds(self, tmp_path):
        ic = _import()
        ic.bootstrap(tmp_path)
    ...
    def test_session_log_has_session_0(self, tmp_path):
        ic = _import()
        ic.bootstrap(tmp_path)
        content = (tmp_path / "NIGHTSHIFT_LOG.md").read_text()
...
class TestInitResult:
    def _make_result(self):
        ...
        r.created = ["nightshift.toml", "CHANGELOG.md"]
        r.skipped = ["NIGHTSHIFT_LOG.md"]
        return r
    ...
    def test_to_markdown_shows_skipped(self):
        ...
        assert "NIGHTSHIFT_LOG.md" in md

    def test_to_markdown_empty(self):
        ic = _import()
        r = ic.InitResult(repo_path="/tmp/empty")
        md = r.to_markdown()
        assert "nightshift health" in md  # still shows the tip
