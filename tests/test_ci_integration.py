"""CI integration tests for the Nightshift CLI pipeline.
...
def _cli(*args: str, timeout: int = 30) -> subprocess.CompletedProcess:
    """Run a nightshift CLI command via ``python -m src.cli``."""
...
class TestCLIHelp:
    ...
    def test_help(self):
        ...
        assert "nightshift" in r.stdout.lower() or "usage" in r.stdout.lower()
