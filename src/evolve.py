...
GAP_AREAS = [
    ...
    GapArea(
        ...
        current_state="Memory = AWAKE_LOG.md (flat text)",
    ...
    GapArea(
        ...
        target_state="awake run --goal 'improve security coverage' guides the session",
        gap_severity="MINOR",
        sessions_to_close=2,
    ),
    GapArea(
        ...
        target_state="awake ci produces GitHub Actions workflow; health gate on PRs",
        gap_severity="SIGNIFICANT",
        sessions_to_close=1,
    ),
    GapArea(
...
PROPOSALS = [
    ...
    EvolutionProposal(
        ...
        example_command="awake health --parallel",
        session_estimate=1,
    ),
    EvolutionProposal(
        title="CI/CD health gate",
        description=(
            "Generate a GitHub Actions workflow that runs `awake audit` on every PR "
        ...
        example_command="awake ci --generate",
        session_estimate=1,
    ),
    EvolutionProposal(
        ...
        example_command="awake test-gen src/new_module.py",
        session_estimate=1,
    ),
    EvolutionProposal(
        ...
        example_command="awake health --watch --webhook https://hooks.slack.com/...",
        session_estimate=1,
    ),
    # TIER 2
    EvolutionProposal(
        ...
        example_command="awake test-heal",
        session_estimate=2,
    ),
    EvolutionProposal(
        ...
        example_command="awake health --repos repo1,repo2,repo3 --compare",
        session_estimate=3,
    ),
    EvolutionProposal(
        title="Persistent semantic memory",
        description=(
            "Replace AWAKE_LOG.md as sole memory store with a SQLite-backed semantic index. "
            "Every decision, pattern, and insight gets indexed for semantic search. "
            "`awake recall 'test coverage'` retrieves relevant past sessions."
        ),
        tier=2, impact="HIGH", effort="HIGH", category="Intelligence",
        rationale="The flat log works for 18 sessions. At 50+, semantic recall becomes necessary.",
        example_command="awake recall 'security improvements'",
        session_estimate=2,
    ),
    EvolutionProposal(
        ...
        description=(
            ...
            "`awake run --goal 'improve security coverage'` re-weights brain signals."
        ),
        ...
        example_command="awake brain --goal 'reduce complexity in core modules'",
        session_estimate=2,
    ),
    # TIER 3
    EvolutionProposal(
        ...
        example_command="awake health --workers 8 --distributed",
        session_estimate=4,
    ),
    EvolutionProposal(
        ...
        example_command="awake health --lang typescript",
        session_estimate=5,
    ),
    EvolutionProposal(
        ...
        example_command="awake review-pr --pr 42",
        session_estimate=3,
    ),
]
...
def save_evolution(report, path):
    ...
    content = "# Awake Evolution Proposals\n\nGenerated automatically by `awake evolve`.\n\n"
