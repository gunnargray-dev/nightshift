"""Session Timeline — ASCII art visual timeline of all Awake sessions.

Reads AWAKE_LOG.md and renders a beautiful horizontal or vertical ASCII
...
def _parse_log(log_path: Path) -> list[SessionNode]:
    """Parse AWAKE_LOG.md and extract session nodes."""
...
def render_timeline(timeline: "Timeline") -> str:
    ...
    if not timeline.sessions:
        return "# Session Timeline\n\nNo sessions found in AWAKE_LOG.md.\n"

    sessions = timeline.sessions
    total_sessions = len(sessions)
    lines: list[str] = []

    lines.append("# \U0001f319 Awake \u2014 Session Timeline")
    lines.append("")
    lines.append(
    ...
    lines.append(
        f"_Generated by `awake timeline` \u00b7 "
        f"{total_sessions} sessions \u00b7 "
...
def build_timeline(
    ...
    """Parse the AWAKE_LOG.md and build a Timeline object."""
    if log_path is None:
        root = repo_path or Path.cwd()
        log_path = root / "AWAKE_LOG.md"
