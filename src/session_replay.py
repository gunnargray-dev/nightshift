...
entirely from AWAKE_LOG.md.  Given a session number, ``replay()``
...
Usage::
    ...
    r = replay(log_path=Path("AWAKE_LOG.md"), session_number=3)
    print(r.to_markdown())

    all_sessions = replay_all(log_path=Path("AWAKE_LOG.md"))
...
class SessionReplay:
    ...
    def to_markdown(self) -> str:
        ...
        lines += ["", "---", "", "*Replayed from AWAKE_LOG.md by `src/session_replay.py`.*"]
        return "\n".join(lines)
...
def _extract_session_sections(log_text: str) -> dict[int, str]:
    """Split AWAKE_LOG.md into per-session text blocks."""
