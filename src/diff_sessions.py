...
Reads AWAKE_LOG.md and git history to produce a detailed side-by-side
comparison of any two sessions.

CLI
---
    awake diff-sessions 1 16       # Compare sessions 1 and 16
    awake diff-sessions 10 16 --json
"""
    ...
    def to_markdown(self) -> str:
        ...
        if self.new_modules:
            ...
            for m in self.new_modules:
                lines.append(f"- `{m}`")
            lines.append("")
        lines += ["---", f"*Generated by `awake diff-sessions {a} {b}`*"]
        return "\n".join(lines)
